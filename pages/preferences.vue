...
<template>
  <div class="mx-auto max-w-3xl space-y-6">
    <PageHeader :title="t('preferences.title')" :subtitle="t('preferences.subtitle')">
      <template #actions>
        <PPButton size="sm" weight="primary" :disabled="busy" @click="saveAll">
          {{ t('preferences.save') }}
        </PPButton>
      </template>
    </PageHeader>
    <div class="flex flex-wrap items-center gap-2">
      <PPStatus v-if="prefs.loadError" tone="warning">
        {{ t('preferences.syncMissing') }}
      </PPStatus>
      <PPBadge v-else-if="prefs.loaded" tone="neutral">
        {{ t('preferences.revision', { revision: prefs.revision }) }}
      </PPBadge>
      <PPStatus v-if="appearance.syncStatus.value === 'synced'" tone="success">
        {{ t('preferences.appearanceSynced') }}
      </PPStatus>
      <PPStatus v-else-if="appearance.syncStatus.value === 'unavailable'" tone="warning">
        {{ t('preferences.appearanceSyncUnavailable') }}
      </PPStatus>
      <PPBadge v-else tone="neutral">
        {{ t('preferences.appearanceDeviceOnly') }}
      </PPBadge>
    </div>
      ...
      <div class="grid gap-3 sm:grid-cols-2">
        ...
        <div class="rounded-[var(--pp-radius-control)] border border-border px-3 py-2.5">
          ...
          <p class="mt-1 text-xs text-muted">
            {{ t('preferences.themeDarkOnlyHint') }}
          </p>
        </div>
        ...
        <div class="space-y-3 sm:pt-6">
          <PPSwitch v-model="appearance.value.value.reducedMotion" :label="t('preferences.reducedMotion')" /><PPSwitch v-model="appearance.value.value.reducedTransparency" :label="t('preferences.reducedTransparency')" />
        </div>
      </div>
    </PPSection>
      ...
      <div class="flex items-center justify-between gap-4">
        <div>
          <p class="text-sm text-foreground">
            {{ t('preferences.sidebarCollapsed') }}
          </p><p class="mt-0.5 text-xs text-muted">
            {{ t('preferences.sidebarHint') }}
          </p>
        </div><PPSwitch v-model="prefs.data.sidebar.collapsed" />
      </div>
    </PPSection>
    ...
    <PPSection :title="t('preferences.performance')">
      <div class="flex items-center justify-between gap-4">
        <div>
          <p class="text-sm text-foreground">
            {{ t('preferences.lowPerformance') }}
          </p><p class="mt-0.5 text-xs text-muted">
            {{ t('preferences.lowPerformanceHint') }}
          </p>
        </div><PPSwitch v-model="prefs.data.low_performance" />
      </div>
    </PPSection>
    ...
    <PPSection :title="t('preferences.experimental')">
      <div class="flex items-center justify-between gap-4">
        <div>
          <p class="text-sm text-foreground">
            {{ t('preferences.desktopWindow') }}
          </p><p class="mt-0.5 text-xs text-muted">
            {{ t('preferences.desktopWindowHint') }}
          </p>
        </div><PPSwitch v-model="prefs.data.desktop_window.enabled" />
      </div>
    </PPSection>
  </div>
</template>
