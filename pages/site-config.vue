...
watch(() => configState.data.value, (value) => {
  ...
  for (const key of Object.keys(form) as Array<keyof typeof form>) if (typeof content[key] === 'string') form[key] = String(content[key])
}, { immediate: true })
...
<template>
  <div class="space-y-4">
    <PageHeader :title="t('siteConfig.title')" :subtitle="t('siteConfig.subtitle')">
      <template #actions>
        <PPStatus :tone="canBuild ? 'success' : 'neutral'">{{ t(canBuild ? 'siteConfig.buildReady' : 'siteConfig.buildMissing') }}</PPStatus>
        <PPButton size="sm" weight="secondary" @click="advancedOpen = true">{{ t('siteConfig.advanced') }}</PPButton>
      </template>
    </PageHeader>
    <AsyncState :loading="configState.loading.value" :error="configState.error.value" :empty="false">
      <PPSection :title="t('siteConfig.siteIdentity')">
        ...
        <div class="mt-5 flex flex-wrap items-center justify-between gap-3 border-t border-border pt-4">
          ...
          <div class="flex gap-2"><PPButton size="sm" weight="secondary" :disabled="busy || !canSave" @click="saveOnly">{{ t('siteConfig.save') }}</PPButton><PPButton size="sm" weight="primary" :disabled="busy || !canSave || !canBuild" @click="saveAndBuild">{{ t('siteConfig.saveBuild') }}</PPButton></div>
        </div>
      </PPSection>
    </AsyncState>
    <PPModal :open="advancedOpen" :title="t('siteConfig.advanced')" width="max-w-xl" @close="advancedOpen = false">
      <div class="space-y-4">
        <div class="grid grid-cols-1 gap-3 sm:grid-cols-2"><label><span class="mb-1 block text-xs">{{ t('siteConfig.google') }}</span><PPInput v-model="form.search_verification_google" /></label><label><span class="mb-1 block text-xs">{{ t('siteConfig.bing') }}</span><PPInput v-model="form.search_verification_bing" /></label></div>
        ...
        <p class="text-xs text-muted">{{ t('siteConfig.advancedHint') }}</p>
      </div>
      <template #footer><div class="flex justify-end gap-2"><PPButton weight="quiet" @click="advancedOpen = false">{{ t('common.close') }}</PPButton><PPButton weight="primary" :disabled="busy || !canSave" @click="saveOnly">{{ t('common.save') }}</PPButton></div></template>
    </PPModal>
