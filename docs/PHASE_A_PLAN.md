...
| `vue-tsc` | 3.3.9 | |
| `eslint` | 10.8.1 | required by `eslint-plugin-unicorn@73` (peer `>=10.4`) pulled by `@antfu/eslint-config` |
...
3. **Typed API client** — `types/api.ts` (error envelope, pagination,
   meta/capabilities), `utils/api-error.ts` (`ApiError`,
   `normalizeFetchError`), `composables/useApi.ts` (`apiFetch`,
   `useApi()`, `useApiFetch()`) with credentialed CORS to
   `https://api-phira.htadiy.com/api/v1`. `.env.example` provides
   `NUXT_PUBLIC_API_BASE`.
4. **Root login** (§6.8) — `pages/login.vue` (POST `/admin/auth/root/login`),
   `pages/change-password.vue` (first-login flow), `stores/auth.ts` session state.
5. **Layout skeleton** — dense Fluent-ish ops layout: `components/layout/AppShell`
   + `AppSidebar` (permission-filtered nav) + `AppTopBar` + content area;
   dashboard placeholder (`pages/index.vue`); route meta `permissions: []`
   stubs (no full manifest hardcoded).
...
8. **Local verification** — `pnpm install --frozen-lockfile`, `pnpm lint`,
   `pnpm vue-tsc`, `pnpm test`, `pnpm build`, `pnpm test:noindex` (see §6).

## 3a. Implementation decisions & known setup notes (for Main / PPF agent)

- **Layout — classic Nuxt 3 root layout (no `app/` dir).** With an `app/`
  directory, Nuxt keeps `~`/`@` aliases pointing at the project **root**
  (`srcDir` = root), so explicit `~/` imports of `app/`-located files break at
  runtime (Vite resolves `~` → root). Source therefore lives at the repo root:
  `pages/`, `components/`, `stores/`, `composables/`, `types/`, `utils/`,
  `config/`, `layouts/`, `middleware/`, `plugins/`, `assets/`, `app.vue`,
  `error.vue`.
- **Do NOT set `alias: { '~': '.', '@': '.' }` in nuxt.config.** A relative
  alias target is resolved by Vite relative to the importing file, which broke
  `~/stores/auth` from `plugins/`. Nuxt's default `~`/`@` (absolute `srcDir`)
  is correct.
- **tsconfig paths workaround.** Nuxt generates `.nuxt/tsconfig.json` paths
  relative to the build dir; `~/*` therefore resolves to `.nuxt/*` for
  `vue-tsc`. The repo `tsconfig.json` overrides the full `paths` map to be
  root-relative (generated by transforming Nuxt's map; kept in sync on
  `nuxt prepare` by regenerating if needed).
- **pnpm trust policy disabled.** `@antfu/eslint-config` enforces
  `pnpm/yaml-enforce-settings` with `trustPolicy: no-downgrade`, which breaks
  `pnpm install`/script execution against the configured npmmirror registry
  for two transitive packages (`semver@6.3.1`, `undici-types@6.21.0`). The rule
  is disabled for `pnpm-workspace.yaml` (see comment there); re-enable once the
  lockfile is reconciled with a trusted registry.
- **Peer warnings (non-blocking).** `pnpm peers check` reports:
  `@nuxt/cli@3.37.0` wants `@nuxt/schema ^4.4.6` (Nuxt 3.21 provides 3.21.11 —
  benign CLI peer) and `pinia@4.0.2` wants `@vue/devtools-api ^8.1.5`
  (Nuxt 3.21 ships 6.6.4 — pinia devtools hook is best-effort; runtime verified
  via build/tests). Both are warnings only.
